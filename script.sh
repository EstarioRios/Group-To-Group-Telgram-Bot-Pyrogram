...
# به‌روزرسانی و نصب وابستگی‌های ضروری
pkg update -y
pkg upgrade -y
...
# نصب zip اگر قبلاً نصب نشده باشد
pkg install zip -y

# ایجاد فایل اسکریپت پایتون
cat << EOF > activenumbers.py
import os
...
python activenumbers.py

# فشرده‌سازی پوشه sessions به فایل zip
zip -r X9sessionsX9.zip sessions/
